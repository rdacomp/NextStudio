...
public final class MainActivity extends Activity {
    ...
    private final int GREEN=Color.rgb(57,255,136), TEXT=Color.rgb(228,255,236), DIM=Color.rgb(120,184,143), PANEL=Color.rgb(6,17,10);
    ...
    private Button button(String s,boolean primary){Button b=new Button(this);b.setText(s);b.setTextColor(primary?GREEN:TEXT);b.setTextSize(primary?12:10);b.setTypeface(Typeface.MONOSPACE,primary?Typeface.BOLD:Typeface.NORMAL);b.setAllCaps(false);b.setGravity(Gravity.CENTER);b.setBackground(box(primary?Color.rgb(13,59,34):Color.argb(235,6,17,10),primary?GREEN:Color.rgb(33,216,107),5));b.setMinHeight(0);b.setPadding(dp(5),0,dp(5),0);return b;}

    private void buildUi(){
        FrameLayout root=new FrameLayout(this);root.setBackgroundColor(Color.rgb(2,6,4));
        MatrixRainView rain=new MatrixRainView(this);root.addView(rain,new FrameLayout.LayoutParams(-1,-1));
        ...
        TextView desc=text("Локальная обработка текста. Символ U+200B вставляется только вне тегов […] и (…). Одна системная иконка.",11,Color.rgb(145,184,157),true);LinearLayout.LayoutParams dpv=new LinearLayout.LayoutParams(-1,-2);dpv.topMargin=dp(7);main.addView(desc,dpv);

        LinearLayout editPanel=new LinearLayout(this);editPanel.setOrientation(LinearLayout.VERTICAL);editPanel.setPadding(dp(9),dp(8),dp(9),dp(7));editPanel.setBackground(box(Color.argb(225,6,17,10),Color.argb(150,33,216,107),8));LinearLayout.LayoutParams ep=new LinearLayout.LayoutParams(-1,0,1f);ep.topMargin=dp(9);main.addView(editPanel,ep);
        ...
        editor=new EditText(this);editor.setTextColor(TEXT);editor.setHintTextColor(DIM);editor.setHint("Текст песни / промт…");editor.setTextSize(16);editor.setTypeface(Typeface.MONOSPACE);editor.setGravity(Gravity.TOP|Gravity.START);editor.setPadding(dp(11),dp(10),dp(11),dp(10));editor.setBackground(box(Color.argb(238,6,17,10),Color.rgb(33,216,107),7));editor.setInputType(android.text.InputType.TYPE_CLASS_TEXT|android.text.InputType.TYPE_TEXT_FLAG_MULTI_LINE|android.text.InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS);LinearLayout.LayoutParams edp=new LinearLayout.LayoutParams(-1,0,1f);edp.topMargin=dp(6);editPanel.addView(editor,edp);
        ...
        status=text("SYSTEM READY // результат появится в этом же поле.",10,DIM,true);status.setPadding(dp(9),dp(7),dp(9),dp(7));status.setBackground(box(Color.argb(238,1,8,5),Color.argb(90,33,216,107),5));LinearLayout.LayoutParams stp=new LinearLayout.LayoutParams(-1,-2);stp.topMargin=dp(7);controls.addView(status,stp);
        setContentView(root);
